...
int main(int argc, char** argv){
  ...
  for(i=0; i<N; i++){
    if(world_rank!=world_size-1){
      ...
      if (world_rank==1){
        MPI_Send(&Vrank_temp, 1, MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
        printf("Hola 4 %d \n", world_rank);
        MPI_Recv(&front_down, 1, MPI_FLOAT, 2, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

        for(j=0;j<n_dis;j++){
          Vrank_temp[matrix(n_dis/4, j, n_dis)]=Vrank[matrix(n_dis/4-1,j,n_dis)]+Vrank[matrix(n_dis/4,j-1, n_dis)]+Vrank[matrix(n_dis/4,j+1, n_dis)]+front_down[j];
        }
        MPI_Recv(&Vrank_temp, 1, MPI_FLOAT, 0, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Send(&Vrank_temp, 1, MPI_FLOAT, 2, 0, MPI_COMM_WORLD);
      }
      if(world_rank==2){
        MPI_Send(&Vrank_temp, 1, MPI_FLOAT, 1, 0, MPI_COMM_WORLD);
        MPI_Recv(&front_down, 1, MPI_FLOAT, 3, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        for(j=0;j<n_dis;j++){
          Vrank_temp[matrix(n_dis/4, j, n_dis)]=Vrank[matrix(n_dis/4-1,j,n_dis)]+Vrank[matrix(n_dis/4,j-1, n_dis)]+Vrank[matrix(n_dis/4,j+1, n_dis)]+front_down[j];
        }
        MPI_Recv(&Vrank_temp, 1, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        MPI_Send(&Vrank_temp, 1, MPI_FLOAT, 3, 0, MPI_COMM_WORLD);
      } printf("hola :)\n" );
    }
    else if(world_rank==world_size-1){
      ...
      MPI_Send(&Vrank_temp, 1, MPI_FLOAT, 2, 0, MPI_COMM_WORLD);
      printf("hola :)\n" );
      MPI_Recv(&Vrank_temp, 1, MPI_FLOAT, 2, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    printf("hola :)\n" );
    }
  ...
  if(world_rank!=0){
    MPI_Send(&Vrank, 1, MPI_FLOAT, 0, 0, MPI_COMM_WORLD);
  }
  printf("Hola \n");
    ...
    if(world_rank==0){
      ...
      MPI_Recv(&V, 1, MPI_FLOAT, 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(&V, 1, MPI_FLOAT, 2, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      MPI_Recv(&V, 1, MPI_FLOAT, 3, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    }
    printf("Hola \n");
if(world_rank==0){
  float dVx, dVy;
